...
void main() {
  ...
  group('VerbatimCollation - Original Text Preservation', () {
    ...
    test('DELETE segment shows text1 original form', () {
      ...
      final deleteChange = changes.firstWhere(
        (c) => c.type == CollationType.delete,
      );
    ...
    test('Mixed scenario - EQUAL, DELETE, INSERT preserve originals', () {
      ...
      if (status == OpenCCStatus.available || status == null) {
        try {
          ...
          // Find segments
          final equalSegments = changes
              .where((c) => c.type == CollationType.equal)
              .toList();
          final deleteSegments = changes
              .where((c) => c.type == CollationType.delete)
              .toList();
          final insertSegments = changes
              .where((c) => c.type == CollationType.insert)
              .toList();
          ...
          if (deleteSegments.isNotEmpty) {
            for (final seg in deleteSegments) {
              expect(
                seg.text,
                isNot(contains('个')),
              ); // Should have 箇 if it's the variant
            }
          }
          ...
          if (insertSegments.isNotEmpty) {
            for (final seg in insertSegments) {
              expect(
                text2.contains(seg.text.replaceAll(RegExp(r'[，！]'), '')),
                isTrue,
              );
            }
          }
    ...
    test('INSERT preserves text2 punctuation even when ignorePunctuation=true', () {
      ...
      expect(
        insertChange.text.contains('！'),
        isFalse,
      ); // Does NOT contain trailing punctuation
    });

    test(
      'INSERT preserves text2 traditional chars even when ignoreTraditional=true',
      () {
        final text1 = '个中';
        final text2 = '个中學問';
        final options = CollationOptions(
        ...
        if (status == OpenCCStatus.available || status == null) {
          try {
            ...
            final insertChange = changes.firstWhere(
              (c) => c.type == CollationType.insert,
              orElse: () =>
                  CollationChange(type: CollationType.equal, text: ''),
            );
    ...
    test(
      'INSERT preserves text2 variant chars even when ignoreVariants=true',
      () {
        final text1 = '个中';
        final text2 = '个中箇人';
        final options = CollationOptions(
    ...
    test(
      'INSERT preserves ALL text2 original features (punctuation + traditional + variants)',
      () {
        final text1 = '个中';
        final text2 = '个中箇學問，深！';
        final options = CollationOptions(
    ...
    test(
      'EQUAL shows text1 form, but text2 originals preserved in non-EQUAL parts',
      () {
        ...
        if (status == OpenCCStatus.available || status == null) {
          try {
            ...
            for (final seg in equalSegments) {
              // 应该包含text1的特征
              final hasText1Features =
                  seg.text.contains('箇') || seg.text.contains('學');
            ...
            if (insertSegments.isNotEmpty) {
              for (final seg in insertSegments) {
                ...
                expect(
                  text2.contains(cleanSeg),
                  isTrue,
                  reason: 'INSERT应该显示text2的原始形式',
                );
    ...
    test(
      'Complex case - both texts have different punctuation and variants',
      () {
  ...
  group('VerbatimCollation - Newline Handling (Issue #4)', () {
    final engine = VerbatimCollation();

    test('Newline as independent character - insertion', () {
      // ABCDE vs ABC\nDE - 增加了一个换行符
      final text1 = 'ABCDE';
      final text2 = 'ABC\nDE';
      final changes = engine.compare(text1, text2);
      ...
      expect(changes[0].type, CollationType.equal);
      expect(changes[0].text, 'ABC');
      expect(changes[1].type, CollationType.insert);
      expect(changes[1].text, '\n');
      expect(changes[2].type, CollationType.equal);
      expect(changes[2].text, 'DE');
    });

    test('Newline as independent character - deletion', () {
      // ABC\nDE vs ABCDE - 删除了一个换行符
      final text1 = 'ABC\nDE';
      final text2 = 'ABCDE';
      final changes = engine.compare(text1, text2);
      ...
      expect(changes[0].type, CollationType.equal);
      expect(changes[0].text, 'ABC');
      expect(changes[1].type, CollationType.delete);
      expect(changes[1].text, '\n');
      expect(changes[2].type, CollationType.equal);
      expect(changes[2].text, 'DE');
    });

    test('Newline should be aligned as EQUAL when separating changes', () {
      // ABC\nDE vs AB\nF
      // 期望：EQUAL(AB) + DELETE(C) + EQUAL(\n) + DELETE(DE) + INSERT(\n) + INSERT(F)
      // 注意：这里的 INSERT(\n) 是错的，应该是 EQUAL(\n)。
      // 修正后的期望：
      // ... DELETE(C) ...
      // EQUAL(\n)  <-- 关键点：换行符应该被对齐并识别为相同
      ...
      final text1 = 'ABC\nDE';
      final text2 = 'ABT\nF'; // Updated comment: T matches nothing in text1?
      // text1: A B C \n D E
      // text2: A B T \n F
      // AB equal.
      // C vs T -> Del(C), Ins(T)
      // \n vs \n -> Equal(\n)
      // DE vs F -> Del(DE), Ins(F)

      final changes = engine.compare(text1, text2);

      // 查找换行符的片段 - 应该有一个 EQUAL 的换行符
      final equalNewlines = changes
          .where((c) => c.text == '\n' && c.type == CollationType.equal)
          .toList();
      expect(
        equalNewlines.length,
        1,
        reason: 'Should have 1 EQUAL newline segment',
      );

      // 验证 'C' 仍然独立
      final hasCSeparate = changes.any(
        (c) => c.type == CollationType.delete && c.text == 'C',
      );
      expect(hasCSeparate, isTrue, reason: 'C should be a separate DELETE');

      // 验证 'DE' 仍然独立
      final hasDESeparate = changes.any(
        (c) => c.type == CollationType.delete && c.text == 'DE',
      );
      expect(hasDESeparate, isTrue, reason: 'DE should be a separate DELETE');

      // 验证 'F' 仍然独立
      final hasFSeparate = changes.any(
        (c) => c.type == CollationType.insert && c.text == 'F',
      );
      expect(hasFSeparate, isTrue, reason: 'F should be a separate INSERT');
    });

    test('Multiple newlines handled independently', () {
      // A\n\nB vs A\nB - 删除了一个换行符
      final text1 = 'A\n\nB';
      final text2 = 'A\nB';
      final changes = engine.compare(text1, text2);
      ...
      // 每个换行符应该独立处理
      final newlineSegments = changes.where((c) => c.text == '\n').toList();
      expect(
        newlineSegments.length,
        greaterThanOrEqualTo(1),
        reason: 'Should have at least one newline segment',
      );

      // 验证 A 和 B 都作为独立片段存在
      expect(
        changes.any((c) => c.text == 'A'),
        isTrue,
        reason: 'A should be a separate segment',
      );
      expect(
        changes.any((c) => c.text == 'B'),
        isTrue,
        reason: 'B should be a separate segment',
      );

      // 验证没有任何片段同时包含换行符和其他字符
      final hasNoMergedNewline = changes.every((c) {
        if (c.text.contains('\n') && c.text.length > 1) {
          return false; // 发现了包含换行符和其他字符的片段
        }
        return true;
      });
      expect(
        hasNoMergedNewline,
        isTrue,
        reason: 'Newlines should not be merged with other characters',
      );
    });
  });
}
